...
namespace SolidworksExecution.Services
{
    ...
    {
        public JObject RunManagedLayoutPlanningHandoff(
            LayoutPlanningHandoffRequest request)
        {
            ...
            if (!attached)
            {
                lifecycle["cleanup"] = ownsSession ? CleanupOwnedSolidWorksSession() :
            ...
            JObject cleanup;
            try
            { result = new LayoutPlanningHandoffExecutor(_solidWorks).Execute(request); }
            finally
            {
            ...
            if (ownsSession && cleanup.Value<string>("status") != "pass")
