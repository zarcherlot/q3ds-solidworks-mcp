...
{
    [RoutePrefix("api/layout-planning/handoff")]
    public sealed class LayoutPlanningHandoffController : ApiController
    {
        ...
        {
            var contract = new LayoutPlanningHandoffContract();
            LayoutPlanningHandoffRequest request;
            LayoutPlanningHandoffContractError error;
                ...
                return Request.CreateResponse(HttpStatusCode.BadRequest, new
                {
                    ...
                    error = new
                    {
                        code = error != null ? error.Code :
                            "LAYOUT_PLANNING_HANDOFF_CONTRACT_INVALID",
            ...
            try
            {
                JObject result = StaExecutor.Instance.Run(() =>
                    _service.RunManagedLayoutPlanningHandoff(request));
                return Request.CreateResponse(HttpStatusCode.OK, result);
            }
            catch (Exception exception)
            {
                ExecLog.Write("<- layout-planning-handoff BLOCKED " +
                    exception.GetType().Name + ": " + exception.Message);
                return Request.CreateResponse(HttpStatusCode.InternalServerError, new
                {
                    status = "blocked",
                    error = new { code = "LAYOUT_PLANNING_HANDOFF_FAILED",
