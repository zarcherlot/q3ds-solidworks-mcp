"""Run the immutable DrawingLayoutPlan G7 matrix through the semantic MCP.

The qualification tools admit planned layout operations only for live evidence.  Known-
unsupported G0 boundaries remain hard blockers, and this runner never edits current.json.
"""
...
from drawing_layout_planner.g7_evidence import (  # noqa: E402
    ...
    build_g7_positive_case_evidence,
    build_g7_summary,
    publish_json_once,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the ten-scenario DrawingLayoutPlan G7 live evidence matrix."
    )
    parser.add_argument("--request", required=True, help="Immutable G7 matrix request JSON")
    parser.add_argument("--summary-output", required=True, help="New immutable summary JSON")
    parser.add_argument("--execution-service-path", required=True)
    parser.add_argument("--execution-pid", required=True, type=int)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Optional new manifest candidate; production manifests are never replaced",
    )
    ...
    try:
        ...
        print(f"G7 live matrix failed closed: {exc}", file=sys.stderr)
        traceback.print_exc()
        return 1
...
async def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    request_path = _existing_json(Path(args.request), "G7 request")
    summary_output = _new_json(Path(args.summary_output), "G7 summary")
    execution_service = _existing_file(
        Path(args.execution_service_path), "execution service"
    )
    capability_manifest = _existing_json(
        Path(args.capability_manifest_path), "layout capability manifest"
    )
    current_manifest = (
        root / "drawing_layout_planner" / "capabilities" / "plan-current.json"
    ).resolve(strict=True)
    if capability_manifest != current_manifest:
        raise ValueError("the semantic server consumes only the repository current manifest")
    if _process_image(args.execution_pid) != execution_service:
        raise RuntimeError("execution PID does not own the requested repository service image")
    ...
    diagnostics = (
        _new_file(Path(args.diagnostics_path), "G7 diagnostics")
    ...
    request = validate_g7_matrix_request(raw_request)
        ...
        async with stdio_client(params, errlog=errlog) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                if names != _EXPECTED_TOOLS:
                    raise RuntimeError(
                        f"unexpected MCP surface; missing={sorted(_EXPECTED_TOOLS - names)}, "
                ...
                for case in request["positive_cases"]:
                    ...
                    if validate.get("execution_readiness") != "capability_blocked":
                        raise RuntimeError(
                            f"{case['case_id']} must remain capability_blocked before G7 promotion"
                        )
                    ...
                    evidence = build_g7_positive_case_evidence(
                        case,
                ...
                for case in request["negative_cases"]:
                    result = await _call(
                        session,
                        "publish_validated_part_drawing_layout_plan",
                        {"request": case["planning_request"]},
                    ...
                    evidence = build_g7_negative_case_evidence(case, result)
                    evidence_path = Path(case["evidence_path"])
                    path, sha256 = publish_json_once(evidence, evidence_path)
                    evidence_paths.append(evidence_path)
                    case_results.append(
    ...
    if protected_before != _snapshot(protected):
        raise RuntimeError("G7 plan, request, qualification, capability, or executor inputs changed")
    summary = build_g7_summary(raw_request, evidence_paths)
    if summary["overall_status"] != "complete":
        raise DrawingLayoutG7EvidenceError("G7 matrix coverage is incomplete")
    ...
    if promotion_output is not None:
        candidate = build_g7_capability_promotion_candidate(
...
async def _call(
    ...
    if result.isError:
        raise RuntimeError(f"MCP tool {name} failed: {' '.join(blocks)}")
    if len(blocks) != 1:
...
def _require_stage(
    payload: Mapping[str, Any], label: str, expected: str, case: Mapping[str, Any]
) -> None:
    if payload.get("ok") is not True or payload.get("status") != expected:
        raise RuntimeError(
    ...
    for field in (
        ...
        if payload.get(field) != case[field]:
            raise RuntimeError(f"{case['case_id']} {label} {field} mismatch")
...
def _protected_files(
    ...
    paths = {
        ...
        Path(request["g0_qualification"]["path"]).resolve(),
    }
    for case in request["positive_cases"]:
        plan_path = Path(case["plan_path"]).resolve()
        paths.add(plan_path)
        _collect_bound_artifacts(_load_json(plan_path), paths)
        _collect_bound_artifacts(case["planning_request"], paths)
    for case in request["negative_cases"]:
        _collect_bound_artifacts(case["planning_request"], paths)
    return sorted(paths, key=lambda path: os.path.normcase(str(path)))


def _collect_bound_artifacts(value: Any, paths: set[Path]) -> None:
    if isinstance(value, Mapping):
        ...
        for child in value.values():
            _collect_bound_artifacts(child, paths)
...
def _process_image(process_id: int) -> Path:
    if sys.platform != "win32":
        raise RuntimeError("G7 execution ownership verification requires Windows")
